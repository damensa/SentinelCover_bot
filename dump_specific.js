...
async function dumpXFA(filename) {
    console.log('Dumping XFA for:', filename);
    ...
    if (xfa instanceof PDFArray) {
        ...
        for (let i = 0; i < xfa.size(); i += 2) {
            ...
            console.log(`Part ${i / 2}: Name=${name}, Ref=${ref}, Length=${stream.contents.length}`);
            fs.writeFileSync(`dumped_part_${i / 2}_${safeName}.bin`, stream.contents);
        }
    } else {
